...
namespace ClientAppTest.Stuff
{
    public class TestGdaApp : IDisposable
    {
        ...
        public Dictionary<string, string> GetGidValues()
        {
            ...
            {
                ModelCode.CONNECTIVITYNODECONTAINER,
            ...
            try
            {
                ...
                List<ModelCode> properties = new List<ModelCode> { ModelCode.IDENTIFIEDOBJECT_GLOBALID, ModelCode.IDENTIFIEDOBJECT_NAME };
                ...
                foreach (var item in concreteClasses)
                {
                    ...
                    while (resourcesLeft > 0)
                    {
                        ...
                        {
                            ids.Add(t.Properties[0].ToString(), t.Properties[1].ToString());
                        }
        ...
        public List<string> GetModelCodesForEntity(ModelCode modelCode)
        {
            ...
            try
            {
                ...
                foreach (var prop in properties.Where(prop => !ids.Contains(prop.ToString())))
                {
            ...
            {
                string message = $"Getting ModelCode values method failed.\n\t{e.Message}";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
        ...
        public List<string> GetReferencesForGid(long gid)
        {
            ...
            try
            {
                var properties = modelResourcesDesc.GetAllPropertyIdsForEntityId(gid);

                ids.AddRange(from prop in properties let chars = ((long) prop).ToString("X").ToCharArray() where chars[15] == '9' select prop.ToString());
            }
            catch (Exception e)
            {
                string message = $"Getting ModelCode values method failed.\n\t{e.Message}";
        ...
        public ResourceDescription GetValues(long globalId, List<ModelCode> modelCodeList)
        {
            ...
            {
                message = $"Getting values method for entered id = {globalId} failed.\n\t{e.Message}";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        public List<long> GetRelatedValues(long sourceGlobalId, Association association, List<ModelCode> properties)
        {
            ...
            try
            {
                ...
                while (resourcesLeft > 0)
                {
                    var rds = GdaQueryProxy.IteratorNext(numberOfResources, iteratorId);
                    foreach (var t in rds)
                    {
                        resultIds.Add(t.Id);
                        t.ExportToXml(xmlWriter);
        ...
        public List<long> GetRelatedValues(long sourceGlobalId, Association association)
        {
            ...
            try
            {
                ...
                properties.Add(ModelCode.IDENTIFIEDOBJECT_ALIASNAME);
